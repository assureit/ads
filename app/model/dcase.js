...
var DCaseDAO = (function (_super) {
    ...
    DCaseDAO.prototype.list = function (page, tagList, callback) {
        ...
        var query = {
            sql: 'SELECT * FROM dcase d, commit c, user u, user cu WHERE d.id = c.dcase_id AND d.user_id = u.id AND c.user_id = cu.id AND c.latest_flag = TRUE AND d.delete_flag = FALSE ORDER BY c.modified DESC, c.id desc LIMIT ? OFFSET ? ',
            nestTables: true
        };
        var params = [
            pager.limit, 
