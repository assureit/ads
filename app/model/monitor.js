...
var MonitorNode = (function () {
    ...
    MonitorNode.tableToObject = function (table) {
...
var MonitorDAO = (function (_super) {
    ...
    MonitorDAO.prototype.get = function (id, callback) {
        ...
        async.waterfall([
            function (next) {
                _this.con.query('SELECT * FROM monitor_node WHERE id=?', [id], function (err, result) {
                    next(err, result);
                });
            },
    ...
    MonitorDAO.prototype.findByThisNodeId = function (dcaseId, thisNodeId, callback) {
        ...
        async.waterfall([
            function (next) {
                _this.con.query('SELECT * FROM monitor_node WHERE dcase_id=? AND this_node_id=?', [dcaseId, thisNodeId], function (err, result) {
                    next(err, result);
                });
            },
            function (result, next) {
    ...
    MonitorDAO.prototype.insert = function (monitor, callback) {
        ...
        async.waterfall([
            function (next) {
                _this.con.query('INSERT INTO monitor_node (dcase_id, this_node_id, watch_id, preset_id, params) VALUES(?,?,?,?,?)', [monitor.dcaseId, monitor.thisNodeId, monitor.watchId, monitor.presetId, JSON.stringify(monitor.params)], function (err, result) {
                    next(err, result);
                });
            }
        ], function (err, result) {
    ...
    MonitorDAO.prototype.update = function (monitor, callback) {
        ...
        async.waterfall([
            function (next) {
                _this.con.query('UPDATE monitor_node SET dcase_id=?, this_node_id=?, watch_id=?, preset_id=?, params=?, rebuttal_this_node_id=?, publish_status=?, delete_flag=? WHERE id=?', [monitor.dcaseId, monitor.thisNodeId, monitor.watchId, monitor.presetId, JSON.stringify(monitor.params), monitor.rebuttalThisNodeId, monitor.publishStatus, monitor.deleteFlag, monitor.id], function (err, result) {
                    next(err);
                });
            }
        ], function (err) {
    ...
    MonitorDAO.prototype.setRebuttalThisNodeId = function (id, rebuttal_id, callback) {
        this.con.query('UPDATE monitor_node  SET rebuttal_this_node_id = ? where id = ?', [rebuttal_id, id], function (err, result) {
            if (err) {
                callback(err);
                return;
            }
    ...
    MonitorDAO.prototype.select = function (id, callback) {
        this.con.query('SELECT dcase_id, this_node_id, rebuttal_this_node_id  from monitor_node where id = ?', [id], function (err, result) {
            if (err) {
    ...
    MonitorDAO.prototype.getLatestCommit = function (dcaseId, callback) {
        this.con.query('SELECT * FROM commit WHERE dcase_id = ? AND latest_flag = TRUE ORDER BY id desc', [dcaseId], function (err, result) {
            if (err) {
                callback(err, null);
                return;
            }
    ...
    MonitorDAO.prototype.getItsId = function (issueId, callback) {
        this.con.query('SELECT its_id FROM issue WHERE id = ?', [issueId], function (err, result) {
            if (err) {
                callback(err, null);
                return;
            }
    ...
    MonitorDAO.prototype.updatePublished = function (monitor, callback) {
        ...
        async.waterfall([
            function (next) {
                _this.con.query('UPDATE monitor_node SET publish_status=? WHERE id=?', [exports.PUBLISH_STATUS_PUBLISHED, monitor.id], function (err, result) {
                    next(err);
                });
            }
        ], function (err) {
    ...
    MonitorDAO.prototype.listNotPublished = function (dcaseId, callback) {
        ...
        async.waterfall([
            function (next) {
                _this.con.query('SELECT * FROM monitor_node WHERE dcase_id=? AND publish_status != ?', [dcaseId, exports.PUBLISH_STATUS_PUBLISHED], function (err, result) {
                    next(err, result);
                });
            },
            function (result, next) {
